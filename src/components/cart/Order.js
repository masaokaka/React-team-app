...
export const Order = () => {
  ...
  const checkMail=(email)=>{//第二引数にコールバック？？？
...
const getUserInfo = ()=>{
  console.log("getUserInfo呼び出し");
    db.collection("users/${:userId}/userinfo").get().then((res)=>{
      console.log(res);
    })
};
  ...
  return (
    <React.Fragment>
      <Router>
        <div>
          <button onClick={getUserInfo}>consoleで確認</button><br/>
          注文確認画面です。
          <form>
            <div>
              <h3>お届け先情報</h3>
              <table>
                <tbody>
                  <tr>
                    ...
                    <td>
                      <input type="text" id='name' onChange={checkName}/>{isError &&<p> 名前を入力してください</p>}
                    </td>
                  </tr>
                  <tr>
                    <td>
                    ...
                    <td>
                      <input type="text" id='mail' onChange={checkMail}/>{isError &&<p> メールアドレスを入力してください</p>}
                    </td>
                  </tr>
                  <tr>
                    <td>
                  ...
                  <tr>
                    ...
                    <td>
                      <label>
                        <input type="radio" name="pay" velue="cash"></input>代金引換
                        <input type="radio" name="pay" velue="creditcard"></input>クレジットカード決済
                      </label>
                    </td>
                  </tr>
                  <tr>
